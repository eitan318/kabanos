...
int ut_symlink_basic(void) {
  ...
  // Create symlink
  UT_ASSERT_SUCCESS(vfs_symlink("/target_file", NULL, "/link_to_target"),
                    "Symlink creation");

  // Read symlink target
  char buf[256];
  ssize_t len = vfs_readlink("/link_to_target", NULL, buf, sizeof(buf));
  if (len < 0)
    return UT_FAIL;
  buf[len] = '\0';
...
int ut_symlink_chain(void) {
  ...
  UT_ASSERT_SUCCESS(vfs_symlink("/real_file", NULL, "/link3"), "link3");
...
int ut_symlink_to_dir(void) {
  UT_ASSERT_SUCCESS(vfs_mkdir("/mydir", NULL, 0755), "Create directory");
  UT_ASSERT_SUCCESS(vfs_create("/mydir/file.txt", NULL, 0644), "Create file");
...
int ut_symlink_broken(void) {
  UT_ASSERT_SUCCESS(vfs_symlink("/nonexistent", NULL, "/broken_link"),
                    "Create broken symlink");

  char buf[256];
  ssize_t len = vfs_readlink("/broken_link", NULL, buf, sizeof(buf));
  if (len < 0)
    return UT_FAIL;
  buf[len] = '\0';
