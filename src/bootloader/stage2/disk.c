...
bool disk_init(uint8_t drive_number, DiskParams* disk_params) {
    ...
    printf("Disk %x: LBA %u\n", drive_number, disk_params->lba_support);
...
bool disk_read_sectors(const DiskParams* disk_params, uint32_t lba,
    ...
    if (disk_params->lba_support) {
        ...
        for (int i = 0; i < reread; i++) {
            printf("Loading %u sectors from LBA=%u\n", count,
        ...
        for (int i = 0; i < reread; i++) {
            printf("Loading %u sectors from CHS=%u:%u:%u\n", count, cylinder,
                   head, sector);
            if (bios_read_chs(disk_params->drive_id, cylinder, head, sector,
                              count, buffer)) {
                return true;
            }
            bios_disk_reset(disk_params->drive_id);
