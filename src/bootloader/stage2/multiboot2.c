...
void multiboot2_build(uint8_t *buffer, char *cmdline, int module_count,
  ...
  // Command line tag
  //
  mb2_tag_string_t *cmd_tag = (mb2_tag_string_t *)ptr;
  cmd_tag->tag.type = MB2_TAG_CMDLINE;
  cmd_tag->tag.size = sizeof(*cmd_tag) + strlen(cmdline) + 1;
  strcpy(cmd_tag->string, cmdline);
  ptr += MB2_ALIGN(cmd_tag->tag.size);

  //
  // Modules tags
  //
  for (int i = 0; i < module_count; i++) {
    mb2_tag_module_t *mod_tag = (mb2_tag_module_t *)ptr;
    ...
    ptr += MB2_ALIGN(mod_tag->tag.size);
  }

  //
  // Memory map tag
  //
  mb2_tag_mmap_t *mmap_tag = (mb2_tag_mmap_t *)ptr;
  ...
  ptr += MB2_ALIGN(mmap_tag->tag.size);

  //
  // End tag
  //
  mb2_tag_t *end_tag = (mb2_tag_t *)ptr;
  end_tag->type = MB2_TAG_END;
  end_tag->size = sizeof(*end_tag);
  ptr += MB2_ALIGN(end_tag->size);
}

void multiboot2_jump_to_kernel(void *kernel_entry, uint8_t *multiboot2_info) {
  __asm__ volatile("movl $0x36d76289, %%eax\n"
                   "movl %0, %%ebx\n"
                   "jmp *%1\n"
                   :
