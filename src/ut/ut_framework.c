...
int ut_run_suite(ut_test_suite_t* suite, ut_config_t* config) {
    ...
    if (config->verbose && !config->quiet) {
        debugf("\n%s┌─ %s%s\n", UT_COLOR_BLUE, suite->suite_name,
               UT_COLOR_RESET);
    }
    ...
    if (!config->quiet) {
        if (config->verbose) {
            ...
            if (failed > 0) {
                debugf("%s✗ %s:%s ", UT_COLOR_RED, suite->suite_name,
                       UT_COLOR_RESET);
...
int ut_run_suites(ut_test_suite_t* suites, int num_suites,
    ...
    if (config->verbose && !config->quiet) {
        debugf("%s══════════ Running %d Test Suites ══════════%s\n",
               UT_COLOR_CYAN, num_suites, UT_COLOR_RESET);
    }
    ...
    if (!config->quiet) {
        if (config->verbose) {
            ...
            debugf("%s════════════════════════════════════════%s\n",
                   UT_COLOR_CYAN, UT_COLOR_RESET);

            if (total_failed == 0 && total_passed > 0) {
                debugf("%s    ✓ All %d suites passed (%d tests)%s\n",
                       UT_COLOR_GREEN, num_suites, total_tests, UT_COLOR_RESET);
            } else {
                debugf("    %s%d/%d suites passed%s",
                       total_passed == num_suites ? UT_COLOR_GREEN
                                                  : UT_COLOR_RESET,
                ...
                if (total_failed > 0) {
                    debugf(", %s%d failed%s\n", UT_COLOR_RED, total_failed,
                           UT_COLOR_RESET);
                    debugf("%s    Failed suites:%s ", UT_COLOR_RED,
                           UT_COLOR_RESET);
