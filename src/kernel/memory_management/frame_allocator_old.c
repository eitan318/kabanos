...
//   memory_range.start = (memory_range.start + PAGE_SIZE - 1) & ~(PAGE_SIZE -
//   1); memory_range.end = memory_range.end & ~(PAGE_SIZE - 1);
//
...
//
//         for (uint64_t f = start_frame; f < end_frame && f < total_frames;
//         f++) {
//           mark_frame_used(f);
//         }
//       }
