...
void task_setup(TCB *t, void (*entry)(void), TaskMode mode) {
  ...
  vmspace_t *task_vmspace = user_vmspace_creat();
  t->pid = next_pid++;
  t->state = TASK_STATE_NEW;
  t->mode = mode;
  t->cr3 = (uint32_t)task_vmspace->pd_phys;

  // Allocate user stack
  if (!va_alloc_region(task_vmspace->pd, PROCESS_STACK_TOP, PROCESS_STACK_SIZE,
                       PAGE_USER | PAGE_READWRITE, true)) {
    vmspace_destroy(task_vmspace);
