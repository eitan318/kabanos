...
int ut_pcb_create_with_pid(void) {
    ...
    UT_ASSERT_EQUAL(PROCESS_PRIORITY_HIGH, pcb->priority, "Priority should be HIGH");
...
int ut_pcb_state_transitions(void) {
    ...
    UT_ASSERT_EQUAL(PROCESS_STATE_TERMINATED, pcb->state, "State should be TERMINATED");
...
int ut_pcb_context_init(void) {
    ...
    UT_ASSERT_EQUAL(entry_point, pcb->context.eip, "EIP mismatch");
    UT_ASSERT_EQUAL(stack_top, pcb->context.esp, "ESP mismatch");
    UT_ASSERT_EQUAL(stack_top, pcb->context.ebp, "EBP mismatch");
    UT_ASSERT_EQUAL(0x202, pcb->context.eflags, "EFLAGS should enable interrupts");
...
int ut_pcb_with_queue(void) {
    ...
    UT_ASSERT(queue_is_empty(&queue), "Queue should be empty after dequeuing all");
...
static ut_test_case_t pcb_tests[] = {
    UT_TEST(ut_pcb_create_destroy),
    UT_TEST(ut_pcb_create_with_pid),
    UT_TEST(ut_pcb_create_no_name),
    UT_TEST(ut_pcb_state_transitions),
    UT_TEST(ut_pcb_context_init),
    UT_TEST(ut_pcb_multiple_create),
    UT_TEST(ut_pcb_string_conversions),
    UT_TEST(ut_pcb_with_queue),
};
