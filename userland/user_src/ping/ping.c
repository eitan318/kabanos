...
static uint32_t get_arrival_ticks(const sockaddr_ll_t *from) {
    return ((uint32_t)from->sa_mac[2] << 24) |
           ((uint32_t)from->sa_mac[3] << 16) |
           ((uint32_t)from->sa_mac[4] <<  8) |
           ((uint32_t)from->sa_mac[5]);
}
...
// ARP resolution 
static int arp_resolve(int sock_raw, uint8_t *target_ip,
...
static void send_ping(int sock, uint8_t *dst_mac, uint8_t *our_mac,
    ...
    uint8_t pkt[ETHER_HEADER_SIZE + IPV4_HEADER_SIZE + ICMP_HEADER_SIZE + PING_DATA_LEN];
    memset(pkt, 0, sizeof(pkt));
    ...
    // ===== ICMP Header + Data =====
    icmp_packet_t *icmp = (icmp_packet_t*)(&pkt[ETHER_HEADER_SIZE + IPV4_HEADER_SIZE]);
    ...
    uint32_t pkt_len = ETHER_HEADER_SIZE + IPV4_HEADER_SIZE + ICMP_HEADER_SIZE + PING_DATA_LEN;
