...
static int copy_file(MyfsSuperBlock *sb, MyfsDiskInode *parent_dir,
  ...
  MyfsDiskInode *inode = myfs_iget(sb, new_ino);
  if (!inode) {
    fclose(f);
    return -1;
  }
...
static int populate_from_dir(MyfsSuperBlock *sb, MyfsDiskInode *fs_parent_dir,
  ...
  while ((ent = readdir(d)) != NULL) {
    ...
    if (S_ISDIR(st.st_mode)) {
      ...
      MyfsDiskInode *child_dir = myfs_iget(sb, new_dir_ino);
      if (!child_dir)
        return -1;
...
int main(int argc, char *argv[]) {
  ...
  if (myfs_format(&dev, host_plt, total_blocks) < 0) {
  ...
  if (partroot) {
    ...
    MyfsDiskInode *root = myfs_iget(sb, sb->on_disk.root_inode);
    if (root == NULL) {
      return -1;
    }
